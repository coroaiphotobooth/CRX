...
import { Upload, X, Image as ImageIcon, Wand2, Download, Save, RefreshCw, Clock, FlaskConical } from 'lucide-react';
...
const CreateView: React.FC<CreateViewProps> = ({ onSaved, onError, onNavigateToSample }) => {
  // State
  ...
  const handleConfirmSave = () => {
    ...
    saveGalleryItem({
      ...
      aspectRatio,
      prompt,
      resultDataUrl: result.resultBase64,
      title: saveTitle.trim()
    });
    
    setIsSaved(true);
    setShowSaveModal(false);
    onSaved(); // Trigger toast in parent
  };
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 rounded-xl">
            <div className="bg-cyber-900 border border-cyber-500 rounded-xl p-4 w-full max-w-sm shadow-2xl">
                <h3 className="text-lg font-bold text-white mb-3 flex items-center gap-2">
                    <Save size={18} className="text-cyber-500"/> Simpan
                </h3>
                <div className="space-y-3">
                    ...
                    <div className="flex gap-2 justify-end">
                        ...
                        <button onClick={handleConfirmSave} className="px-4 py-1.5 rounded text-xs bg-cyber-500 text-white font-bold">Simpan</button>
                    </div>
                </div>
            </div>
        </div>
